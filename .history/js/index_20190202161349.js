...
class UI {
  ...
  static calculateTotal(calculate) {
    // var quantity = document.querySelector("#fQua");
    // var price = document.querySelector("#fPrice");
    // calculate;
    var list = document.querySelector("product-list");
    var row = document.createElement("TR");
    row.innerHTML = `<td>${product.total}</td>`;
    list.appendChild(row);
}
...
function getPrice(result) {
  ...
  result = getQuantity * getInput;
   ...
   if (!result) {
     document.getElementById("product-list")[0].innerHTML = "Please Enter Numbers";
   } else {
     document.getElementById("product-list")[0].innerHTML = "Total price is = $" + result;
   }
}

document.querySelector("#product-form").addEventListener("click", getPrice);
...
// document.querySelector("#product-form").addEventListener("submit", getPrice);
