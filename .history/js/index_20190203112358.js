...
function getTotalPrice(itemNode) {
  ...
  var totalPrice = document.querySelector("#sumtotalP span");
  totalPrice.innerText = 0;

  for (var i = 0; i < products.length; i++) {
    products[i].querySelector("#price").innerText = Number(Number(products[i]
      .querySelector("#price").innerText) * Number(products[i].querySelector("#quantity").value));
      totalPrice.innerText = Number(totalPrice.innerText) + Number(products[i].querySelector("#price").innerText);
      // if (!total) {
